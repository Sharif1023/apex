...
const App: React.FC = () => {
  ...
  const handleProcessPayment = () => {
    ...
    setTimeout(() => {
      const orderId = `#APX-${Math.floor(1000 + Math.random() * 8999)}`;
  ...
  return (
    ...
    >
      {currentView === 'admin' ? (
        <AdminPanel products={products} setProducts={setProducts} categories={categories} setCategories={setCategories} orders={[]} setOrders={() => {}} />
      ) : (
